<template>
  <div class="schedule-container">
    ...
    <div class="schedule-header">
      <div class="header-content">
        ...
        <div class="header-actions">
          <div class="filters-group">
            <select v-model="selectedDepartment" class="elite-select department-select">
            ...
            <select v-model="selectedPromotion" class="elite-select">
              <option value="">Toutes les promotions</option>
              <option v-for="promo in filteredPromotions" :key="promo._id" :value="promo._id">
                {{ promo.name }}
              </option>
            </select>

            <select v-model="selectedProfessor" class="elite-select">
          ...
          <div class="action-buttons">
            <button class="btn-secondary" @click="refreshSchedules" :disabled="isRefreshing">
      ...
      <div class="schedule-controls">
        <div class="schedule-tabs">
          <button v-for="dept in departments" :key="dept.id" class="department-tab"
            :class="{ 'active': activeDepartments.includes(dept.id) }" @click="toggleDepartment(dept.id)"
            :style="{ '--dept-color': getDepartmentColor(dept.id) }">
            <div class="tab-indicator" :style="{ background: getDepartmentColor(dept.id) }"></div>
            <span>{{ dept.name }}</span>
            <Icon
              :icon="activeDepartments.includes(dept.id) ? 'material-symbols:visibility-outline' : 'material-symbols:visibility-off-outline'" />
          </button>
        </div>
            ...
            <template v-if="displayMode === 'overlay'">
              <div v-for="(schedule, index) in getCoursesForSlot(day.date, hour)" :key="schedule._id"
                ...
                <div v-if="schedule.status === 'done'" class="done-course-indicator">
                  ...
                  <div v-if="schedule.approvedByCron" class="done-icon robot">
                    <Icon icon="material-symbols:android" />
                  </div>
                  <!-- Humain vert si approuvé manuellement -->
                  <div v-else class="done-icon human">
              ...
              <div class="courses-container">
                <div v-for="(schedule, index) in getCoursesForSlot(day.date, hour)" :key="schedule._id"
                  ...
                  <div v-if="schedule.status === 'done'" class="done-course-indicator">
                    <!-- Robot vert si approuvé par cron -->
                    <div v-if="schedule.approvedByCron" class="done-icon robot"
                      :class="{ 'compact': displayMode === 'sideBySide' }">
                      <Icon icon="material-symbols:android" />
                    </div>
                    <!-- Humain vert si approuvé manuellement -->
                    <div v-else class="done-icon human" :class="{ 'compact': displayMode === 'sideBySide' }">
                      <Icon icon="material-symbols:verified-user" />
                    </div>
                  </div>
    ...
    <!-- Modal détails cours -->
    <ScheduleModalView :is-open="!!selectedSchedule" :course="selectedSchedule" @close="selectedSchedule = null"
      @edit="openEditCourse" @delete="deleteCourse" :promotions="availablePromotions"
      @approved="ApprovedStatusManually" />

    <!-- Modal planification -->
    <ScheduleFormModal v-if="modules.length > 0" :is-open="showFormModal" :selected-date="selectedModalDate"
      :selected-time="selectedModalTime" :Schedule="editingSchedule" :module="modules" @close="closeFormModal"
      @save="handleSaveSchedule" :professors="professors" :promotions="availablePromotions" />
    ...
    <div class="schedule-legend">
      ...
      <div class="legend-section">
        <h4>Matières</h4>
        <div class="legend-items">
          <div v-for="module in modules" :key="module._id" class="legend-item">
            <div class="legend-color" :class="`subject-${module.code}`"></div>
      ...
      <div class="legend-section">
        ...
        <div class="legend-items">
          ...
          <div class="legend-item">
            <div class="legend-indicator done-robot-demo">
              <Icon icon="material-symbols:android" />
            </div>
            <span>Cours terminé - Validation automatique</span>
          </div>
          <div class="legend-item">
            <div class="legend-indicator done-human-demo">
              <Icon icon="material-symbols:verified-user" />
            </div>
            <span>Cours terminé - Validation manuelle</span>
          </div>
        </div>
      </div>
    </div>
...
import ScheduleFormModal from '@/components/admin/Schedule/ScheduleFormModal.vue'
import ScheduleModalView from '@/components/admin/Schedule/ScheduleModalView.vue'
...
const availablePromotions = ref<PromotionInterface[]>([])
...
const refreshSchedules = async () => {
  ...
  try {
    // Recalculer la période affichée
    getWeekRangeForDB()

    // Invalider le cache de la période en cours, puis recharger
    ...
    toast.value = { show: true, message: "Emploi du temps actualisé.", type: 'success', title: 'Succès' }
  } catch (error: unknown) {
    const message = error instanceof AxiosError ? error.response?.data : 'Une erreur est survenue'
...
const ApprovedStatusManually = async (id: string) => {
  try {
    const response = (await axios.patch(`/api/v1/schedules/status/${id}`)).data
    ...
    emit("SendNotificationForscheduleReclamationByAdmin", ({ schedule: selectedSchedule.value }));

    toast.value = { show: true, message: 'Le cours a été approuvé manuellement.', type: 'success', title: 'Succès' }
  } catch (error: unknown) {
    const message = error instanceof AxiosError ? error.response?.data : 'Une erreur est survenue';
    toast.value = { show: true, message, type: 'error', title: 'Erreur' }
...
const getAllActiveAvailablePromotions = async () => {
  try {
    const response = (await axios.get("/api/v1/promotions/active")).data

    availablePromotions.value = response

  } catch (error) {
    toast.value = { show: true, message: error instanceof AxiosError ? error.response?.data : 'Une erreur est survenue lors de la récupération des promotions.', type: 'error', title: 'Erreur' }
...
const filteredPromotions = computed(() => {
  if (!selectedDepartment.value) return availablePromotions.value
  const departmentName = getDepartmentName(selectedDepartment.value).toLowerCase()
  return availablePromotions.value.filter(promo =>
    promo.field.toLowerCase().includes(departmentName) ||
    departmentName.includes(promo.field.toLowerCase())
  )
})
...
const toggleDepartment = (deptId: string) => {
  const index = activeDepartments.value.indexOf(deptId)
  if (index > -1) {
    activeDepartments.value.splice(index, 1)
  } else {
    activeDepartments.value.push(deptId)
  }
}
...
const openEditCourse = (schedule: ScheduleInterface) => {
  openEditModal(schedule)
}

const openAddModal = (date?: Date, hour?: string) => {
  if (modules.value.length === 0) {
    toast.value = { show: true, message: 'Veuillez d\'abord ajouter des modules avant de créer un emploi du temps.', type: 'error', title: 'Erreur' }
    return
  }
...
const openEditModal = (schedule: ScheduleInterface) => {
  editingSchedule.value = schedule
  selectedModalDate.value = undefined
  selectedModalTime.value = undefined
  showFormModal.value = true
}
...
const fetchAllAvailableModule = async () => {
  try {
    const response = await axios.get('/api/v1/modules/all')
    modules.value = response.data
  } catch (error) {
    toast.value = { show: true, message: error instanceof AxiosError ? error.response?.data : 'Une erreur est survenue lors de la récupération des modules.', type: 'error', title: 'Erreur' }
...
const fetchAllProfessors = async () => {
  try {
    ...
    toast.value = { show: true, message: error instanceof AxiosError ? error.response?.data : 'Une erreur est survenue lors de la récupération des professeurs.', type: 'error', title: 'Erreur' }

  }
}


type ScheduleFormValues = Omit<ScheduleInterface, 'module'> & { module?: string | ModuleInterface | null }

const handleSaveSchedule = async (values: ScheduleFormValues) => {
  const moduleId = typeof values.module === 'string' ? values.module : values.module?._id
  const module = moduleId ? modules.value.find((m) => m._id === moduleId) : null
  try {
    if (editingSchedule.value) {
      // Edition
      const response = await axios.patch(`/api/v1/schedules/${editingSchedule.value._id}`, values);
      const message = values.module
        ? `L'emploi du temps pour le module ${module?.code} : ${values.title}, le ${new Date(values.date).toLocaleDateString('fr-FR')} a été mis à jour.`
        : `L'emploi du temps : ${values.title}, le ${new Date(values.date).toLocaleDateString('fr-FR')} a été mis à jour.`;
      emit('sendScheduleNotification', { message: message, schedule: response.data, oldScheduleValue: editingSchedule.value });
      const index = schedules.value.findIndex(s => s._id === editingSchedule.value?._id);
      if (index !== -1) {
        schedules.value[index] = response.data;
        selectedSchedule.value = response.data;

      }
      // Invalider le cache pour la date modifiée
      invalidateCache(new Date(values.date));
      toast.value = { show: true, message: 'Emploi du temps mis à jour avec succès !', type: 'success', title: 'Succès' }
      closeFormModal()


      return
    }
    const response = await axios.post("/api/v1/schedules", values);
    const message = values.module
      ? `Un nouvel emploi du temps a été planifié pour le module ${module?.code} : ${values.title}, le ${new Date(values.date).toLocaleDateString('fr-FR')}, de ${values.startTime} à ${values.endTime}.`
      : `Un nouvel emploi du temps a été planifié : ${values.title}, le ${new Date(values.date).toLocaleDateString('fr-FR')}, de ${values.startTime} à ${values.endTime}.`;


    emit('sendScheduleNotification', { message: message, schedule: response.data, oldScheduleValue: null });
    schedules.value.push(response.data);
    // Invalider le cache pour la nouvelle date ajoutée
    invalidateCache(new Date(values.date));
    toast.value = { show: true, message: 'Emploi du temps ajouté avec succès !', type: 'success', title: 'Succès' }
    closeFormModal()
    return
...
const deleteCourse = async (course: ScheduleInterface) => {
  if (!confirm('Êtes-vous sûr de vouloir supprimer cet emploi du temps ?')) {
    return
  }

  try {
    await axios.delete(`/api/v1/schedules/${course._id}`);
    toast.value = { show: true, message: 'Emploi du temps supprimé avec succès !', type: 'success', title: 'Succès' }

    // Invalider le cache pour la semaine de ce cours
    invalidateCache(new Date(course.date))

    // Retirer le cours de la liste locale
    const index = schedules.value.findIndex(s => s._id === course._id);
    if (index !== -1) {
      schedules.value.splice(index, 1);
    }

    // Notifier via socket
    emit('cancelScheduleNotification', course);
  ...
  try {
    ...
    if (cacheKey && scheduleCache.value.has(cacheKey)) {
      ...
      if (isCacheValid(cachedData.timestamp)) {
        ...
        // Cache stale mais utilisable - servir immédiatement et revalider en arrière-plan

        // Déclencher la revalidation en arrière-plan
    ...
    // Faire la requête API si pas de cache utilisable
    const response = (await axios.get("/api/v1/schedules/by-range", {
      params: {
...
// Fonction pour invalider le cache (appelée lors des modifications)
const invalidateCache = (scheduleDate?: Date) => {
  if (scheduleDate) {
    // Invalider seulement le cache pour la semaine contenant cette date
    const weekStart = new Date(scheduleDate)
    weekStart.setDate(weekStart.getDate() - weekStart.getDay() + 1)
    const weekEnd = new Date(weekStart)
    weekEnd.setDate(weekEnd.getDate() + 5)

    const cacheKey = generateCacheKey(weekStart, weekEnd)
    if (scheduleCache.value.has(cacheKey)) {
      scheduleCache.value.delete(cacheKey)
    }
  } else {
...
watch([currentWeek, viewMode], async () => {
  getWeekRangeForDB()
  await getWeekSchedules()
})
...
onMounted(() => {
  ...
  checkAndAdvanceWeek() // Vérifier si on doit avancer à la semaine suivante
  fetchAllAvailableModule()
  fetchAllProfessors()
  getAllActiveAvailablePromotions()


  // Vérifier périodiquement si on doit changer de semaine (toutes les heures)
  const weekCheckInterval = setInterval(() => {
    checkAndAdvanceWeek()
  }, 1000 * 60 * 60) // Chaque heure
