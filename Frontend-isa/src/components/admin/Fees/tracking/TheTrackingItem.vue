      ...
      <div class="elite-modal-content">
        ...
        <div class="elite-student-card">
          <div class="elite-student-avatar"
            :style="{ backgroundImage: `url('/api/v1/document?fullPath=${encodeURIComponent(tutionFees.user.identityPhoto)}')` }">
          </div>
          <div class="elite-student-info">
            <h4>{{ tutionFees.user.firstName }} {{ tutionFees.user.name }}</h4>
            <div class="elite-student-meta">
              <span class="elite-badge">
                <Icon icon="mdi:identifier" width="14" height="14" />
                {{ tutionFees.user.matricule }}
              </span>
              <span class="elite-badge">
                <Icon icon="mdi:bookshelf" width="14" height="14" />
                {{ tutionFees.user.field }}
              </span>
              <span class="elite-badge">
                <Icon icon="mdi:stairs" width="14" height="14" />
                {{ tutionFees.user.level }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="elite-payment-summary">
          ...
          <div class="elite-summary-card">
            ...
            <div>
              ...
              <strong>{{ tutionFees.totalAmount?.toLocaleString() || '0' }} Ar</strong>
            </div>
          </div>
        </div>

        <!-- Liste des échéances -->
        <div class="elite-installments-list" v-if="tutionFees.installments.length">
          <h4 class="elite-section-title">
            <Icon icon="mdi:calendar-alert" width="20" height="20" />
            Échéances de paiement
          </h4>

          <div v-for="(item, index) in tutionFees.installments" :key="index" class="elite-installment-card"
              ...
              <div class="elite-installment-details" v-if="item.method">
                ...
                <div class="elite-detail-item proof">
                  ...
                  <TheImageVisualizer v-if="item.proofFile" :url="`/api/v1/document?fullPath=${item.proofFile}`"
                    type="image" :title="`${item.label} - ${tutionFees.user.matricule}`">
                    <template #trigger>
                ...
                <div v-if="!item.verified" class="actions">
                  <ActionButton icon="mdi:check-circle-outline" size="small" icon-size="16"
                    @click="emit('acceptPayment', item.label)">
                    Valider
                  </ActionButton>
                  <ActionButton icon="mdi:check-circle-outline" size="small" icon-size="16" variant="danger"
                    @click="emit('rejectPayment', item.label)">
                    Rejeter
                  </ActionButton>
                </div>
              </div>
...
const props = defineProps<{
  tutionFees: TutionFeesInterface
}>()

const emit = defineEmits<{
  (e: 'close'): void
  (e: 'validStudent', tutionFees: TutionFeesInterface): void
  (e: 'rejectPayment', label: string): void
  (e: 'acceptPayment', label: string): void
}>()
